...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    // Adding new mahasiswa
    void addMahasiswa(Mahasiswa mahasiswa) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_NAME, mahasiswa.getName()); // mahasiswa Name
        values.put(KEY_NPM, mahasiswa.getnpm()); // mahasiswa npm
    ...
    public List<Mahasiswa> getAllMahasiswa() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                mahasiswa.setnpm(cursor.getString(2));
    ...
    public boolean checkUser(String name, String password) {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        // selection criteria
        String selection = KEY_NAME + " = ?" + " AND " + KEY_NPM
                + " = ?";
